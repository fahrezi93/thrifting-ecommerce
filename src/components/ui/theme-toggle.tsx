...
export const ThemeToggleButton = ({ 
  ...
  const handleClick = useCallback(() => {
    ...
    if (variant === 'circle') {
      ...
      css = `
        @supports (view-transition-name: root) {
          ...
          ::view-transition-new(root) {
            animation: circle-blur-expand 0.4s ease-out;
            transform-origin: ${positions[start]};
    ...
    // Optimized theme switching with better performance
    const updateTheme = () => {
    ...
    // Use View Transitions API with feature detection
  ...
  return (
    <Button
      ...
      onClick={handleClick}
      className={cn(
        'relative overflow-hidden transition-all hover:scale-105 active:scale-95',
    ...
    >
      {theme === 'light' ? (
        <Sun className="h-[1.2rem] w-[1.2rem] text-yellow-500 transition-colors" />
      ) : (
        <Moon className="h-[1.2rem] w-[1.2rem] text-blue-400 transition-colors" />
      )}
      {showLabel && (
